    ...
    M: Middleware,
{
    let contract = Contract::new(contract_address, contract_abi.clone(), prov);
    let call: ContractCall<M, _> = contract
        .method::<_, bool>("compute", k)
        .expect("cannot contract keccak256recursive compute call");

    // Set gas to avoid `eth_estimateGas` call
    let call = call.legacy();
    let call = call.gas(5_000_000u64);
    call.tx
}
...
async fn main() {
    ...
    // Keccak256Recursive
    let constrct_bytes32: [u8; 32] = [1, 35, 69, 103, 137, 171, 205, 239, 1, 35, 69, 103, 137, 171, 205, 239, 1, 35, 69, 103, 137, 171, 205, 239, 1, 35, 69, 103, 137, 171, 205, 239];
    let contract = deploy(
        prov_wallet0.clone(),
        contracts.get("Keccak256Recursive")
        .expect("contract not found"),
        constrct_bytes32,
    )
    .await;
    let block_num = prov.get_block_number().await.expect("cannot get block_num");
    blocks.insert(
        "Deploy Keccak256Recursive".to_string(),
        block_num.as_u64(),
    );
    deployments.insert(
        "Keccak256Recursive".to_string(),
        (block_num.as_u64(), contract.address()),
    );
    ...
    // Generating Keccak256Recursive calls
    // ...
    let contract_address = deployments
        .get("Keccak256Recursive")
        .expect("contract not found")
        .1;
    let contract_abi = &contracts
        .get("Keccak256Recursive")
    ...
    // Keccak256Recursive multiple calls in a single block
    info!("Doing Keccak256Recursive multiple computes...");
    cli.miner_stop().await.expect("cannot stop miner");
    let mut tx_hashes = Vec::new();
    for (i, k) in [100, 500, 1000, 3000, 5000].iter().enumerate() {
        let prov_wallet = &wallets[i];
        let tx = keccak256_compute(prov_wallet.clone(), contract_address, contract_abi, U256::from(*k as u64));
        let pending_tx = prov_wallet
            .send_transaction(tx, None)
            .await
            .expect("cannot send keccak256recursive compute call");
        let tx_hash = *pending_tx;
        tx_hashes.push(tx_hash);
    }
    cli.miner_start().await.expect("canot start miner");
    for (i, tx_hash) in tx_hashes.iter().enumerate() {
        let prov_wallet = &wallets[i];
        let pending_tx = PendingTransaction::new(*tx_hash, prov_wallet.inner());
        let receipt = pending_tx.confirmations(0usize).await.unwrap().unwrap();
